...
import intellij.ExternalLibraryDependency;
import intellij.ExternalMultiLibraryDependency;
...
public final class GradleSourceSetDependencyResolver {
	...
	private @NotNull Collection<ExternalDependency> resolveDependencies(
		...
		GradleDependencyResolver dependencyResolver = new GradleDependencyResolver(myProject);
		...
		for (ExternalDependency dependency : dependencies) {
			if (dependency instanceof ExternalProjectDependency) {
				...
				for (File artifactFile : projectDependency.getProjectDependencyArtifacts()) {
					SourceSet sourceSet = null;// todo: mySourceSetArtifactIndex.findByArtifact(artifactFile.getPath());
					if (sourceSet != null) {
